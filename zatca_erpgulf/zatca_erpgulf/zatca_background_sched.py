...
def zatca_call_purchase_invoice_scheduler_background(
    invoice_number,
    compliance_type="0",
    any_item_has_tax_template=False,
    company_abbr=None,
    ...
    based on this the zATCA output and message is getting"""
    try:
        if not frappe.db.exists(PURCHASE_INVOICE, invoice_number):
        ...
        if compliance_type == "0":
            if supplier_doc.custom_b2c == 1:
                ...
                frappe.throw(
                    "Supplier should be B2C purchase invoice with xml during create xml"
                )
        else:
            invoice = invoice_typecode_compliance(invoice, compliance_type)
        ...
        if compliance_type == "0":
            if supplier_doc.custom_b2c == 1:
                ...
                reporting_api_purchase_withoutxml(
                    uuid1,
                    encoded_hash,
                    signed_xmlfile_name,
                    invoice_number,
                ...
                frappe.throw(
                    "Supplier should be B2C type required for simplified invoice"
                )
        else:
            compliance_api_call(
                uuid1,
    ...
    try:
        ...
        if company_doc.custom_send_invoice_to_zatca not in ["Batches", "Background"]:
            ...
            try:
                ...
                frappe.throw(
                    _(f"Error in reporting API-2 sales invoice without xml in zatca background: {str(e)}")
                )
...
def reporting_api_purchase_withoutxml(
    ...
    try:
        ...
        if company_doc.custom_send_invoice_to_zatca not in ["Batches", "Background"]:
            ...
            try:
                ...
                frappe.throw(
                    f"Error in reporting API-2 purchase invoice with xml: {str(e)}"
                )
        ...
        frappe.throw(f"Error in reporting API-1 purchase invoice with xml: {str(e)}")
