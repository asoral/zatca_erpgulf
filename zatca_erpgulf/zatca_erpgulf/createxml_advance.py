...
def parse_csr_config(csr_config_string):
    ...
    csr_config = {}
    lines = csr_config_string.strip().splitlines()
    for line in lines:
        line = line.strip()
...
def compliance_api_call(
    ...
    try:
        ...
        if response.status_code == 200:
            frappe.msgprint(_("ZATCA Compliance successful:<br>{0}").format(response.text), indicator="green")
            return response.text
        elif response.status_code == 202:
            frappe.msgprint(_("ZATCA Compliance returned warnings:<br>{0}").format(response.text), indicator="orange")
            return response.text
        else:
            frappe.throw(_("Error in compliance: {0}").format(response.text))
