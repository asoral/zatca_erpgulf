    ...
    try:
        ...
        if not company_doc.is_group and company_doc.custom_costcenter and company_doc.parent_company:
            ...
            if not company_doc.abbr:
                frappe.throw(
                    _(f"Company with abbreviation {company_doc.name} not found.")
                )
...
def reporting_api_xml_purchase_invoice_simplified(
    ...
    try:
        ...
        send_request_and_handle_response_for_purchase_invoice(
            company_abbr,
            invoice_number,
            payload,
            headers,
...
def send_request_and_handle_response_for_purchase_invoice(
    company_abbr,
    invoice_number,
    payload,
    headers,
    ...
    if response.status_code in (400, 405, 406, 409):
        ...
        handle_successful_submission_for_purchase_invoice(
...
def handle_failed_submission_for_purchase_invoice(invoice_number, response, error_message):
    """handle_failed_submission"""
    update_invoice_status_for_purchase_invoice(invoice_number, NOT_SUBMITTED)
    frappe.throw(
        f"{error_message} Status code: {response.status_code}<br><br>{response.text}"
    )
...
def handle_successful_submission_for_purchase_invoice(
    invoice_number, response, sales_invoice_doc, encoded_hash, uuid1
):
    """handle_successful_submission"""
    msg = (
    ...
    update_company_or_pos_settings(sales_invoice_doc, encoded_hash, msg)
    update_invoice_status_for_purchase_invoice(invoice_number, "REPORTED", uuid1, msg)
    success_log(response.text, uuid1, invoice_number)
...
def update_invoice_status_for_purchase_invoice(invoice_number, status, uuid1=None, msg=None):
    """update_invoice_status"""
...
def submit_purchase_invoice_simplifeid(purchase_invoice_doc, file_path, invoice_number):
    ...
    try:
        ...
        frappe.throw(_(f"Error in submitting purchase invoice in simplified: {str(e)}"))
